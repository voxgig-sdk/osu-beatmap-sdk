...
func TestBeatmapEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OSUBEATMAP_TEST_BEATMAP_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		beatmapRef01DataDt0LoadResult := core.ToMapAny(beatmapRef01DataDt0Loaded)
...
func beatmapBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OSUBEATMAP_TEST_BEATMAP_ENTID")
