...
import (
	...
	omni "github.com/voxgig-sdk/osu-beatmap-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OsuBeatmapError); is {
				out := map[string]any{
					"name":    "OsuBeatmapError",
...
func omniCtx(arg any, client *sdk.OsuBeatmapSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OsuBeatmapSDK); is {
			client = live
