...
boru-driven command-line client **and** interactive REPL for the OsuBeatmap
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/osu-beatmap-cli)
make build

# 2. See usage (words, entities, env vars)
./osu-beatmap-cli --help

# 3. Provide credentials once, via the environment
export OSU_BEATMAP_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./osu-beatmap-cli load 1 beatmap            # {id:1} shorthand
./osu-beatmap-cli load '{id:1}' beatmap       # explicit match map

# 5. Override the API base URL for a single call
OSU_BEATMAP_BASE=https://api.example.com ./osu-beatmap-cli load 1 beatmap

# 6. No arguments -> interactive REPL
./osu-beatmap-cli
osu-beatmap> load 1 beatmap
osu-beatmap> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/osu-beatmap-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/osu-beatmap-cli load 1 beatmap
   ```
...
```sh
./osu-beatmap-cli load 1 beatmap          # scalar shorthand for {id:1}
./osu-beatmap-cli load '{id:1}' beatmap     # explicit match map
```
...
./osu-beatmap-cli load 1 beatmap
```
...
Run with no arguments to open a REPL (prompt `osu-beatmap>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `beatmap`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/osu-beatmap-cli`. |
...
(`./osu-beatmap-cli <expr>`) and interactively (the REPL), and expressions compose the
