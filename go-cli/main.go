// Command OsuBeatmap-cli is a CLI + REPL for the OsuBeatmap SDK,
...
const prompt = "osu-beatmap"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "beatmap download search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OSU_BEATMAP_APIKEY (api key), OSU_BEATMAP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OSU_BEATMAP_APIKEY carries the API key and
	// OSU_BEATMAP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OSU_BEATMAP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OSU_BEATMAP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOsuBeatmapSDK(opts)
