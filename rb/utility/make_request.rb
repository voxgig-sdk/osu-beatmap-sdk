...
module OsuBeatmapUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OsuBeatmapResponse.new({})
    ...
    if fetch_err
      ...
      response = OsuBeatmapResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OsuBeatmapResponse.new(fetched)
    else
