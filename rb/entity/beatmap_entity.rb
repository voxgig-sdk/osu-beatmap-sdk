...
class BeatmapEntity
  ...
  def make
    opts = @_entopts.dup
    BeatmapEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OsuBeatmapHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = OsuBeatmapHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Beatmap.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = OsuBeatmapHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
