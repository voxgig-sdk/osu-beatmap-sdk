...
class BeatmapDirectTest < Minitest::Test
  def test_direct_load_beatmap
    setup = beatmap_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-beatmap", setup[:live] ? "live" : "unit")
    if _should_skip
...
def beatmap_direct_setup(mockres)
