...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = OsuBeatmapOmni.make_runner(TEST_JSON_FILE, OsuBeatmapSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    OsuBeatmapError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : OsuBeatmapOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = OsuBeatmapOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
