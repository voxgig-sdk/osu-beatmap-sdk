...
class TestBeatmapDirect:

    def test_should_direct_load_beatmap(self):
        setup = _beatmap_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-beatmap", "live" if setup["live"] else "unit")
        if _skip:
...
def _beatmap_direct_setup(mockres):
    ...
    live = env.get("OSU_BEATMAP_TEST_LIVE") == "TRUE"
