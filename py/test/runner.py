...
class OsuBeatmapTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OsuBeatmapTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OsuBeatmapTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OsuBeatmapTestRunner.getenv("OSU_BEATMAP_TEST_LIVE")
        override = OsuBeatmapTestRunner.getenv("OSU_BEATMAP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OsuBeatmapTestRunner.getenv(key)
        ...
        explain = OsuBeatmapTestRunner.getenv("OSU_BEATMAP_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if OsuBeatmapTestRunner._test_control is not None:
            return OsuBeatmapTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OsuBeatmapTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OsuBeatmapTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OsuBeatmapTestRunner.load_test_control()
...
def load_env_local():
    OsuBeatmapTestRunner.load_env_local()


def env_override(m):
    return OsuBeatmapTestRunner.env_override(m)


def entity_data(v):
    return OsuBeatmapTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OsuBeatmapTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OsuBeatmapTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OsuBeatmapTestRunner.load_test_control()


def live_client_options():
    return OsuBeatmapTestRunner.live_client_options()


def live_delay_ms():
    return OsuBeatmapTestRunner.live_delay_ms()
