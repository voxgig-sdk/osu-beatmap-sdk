...
class TestBeatmapEntity:

    def test_should_create_instance(self):
        testsdk = OsuBeatmapSDK.test(None, None)
        ent = testsdk.Beatmap(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _beatmap_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "beatmap." + _op, "live" if _live else "unit")
            if _skip:
        ...
        beatmap_ref01_data = None
        if len(beatmap_ref01_data_raw) > 0:
            beatmap_ref01_data = helpers.to_map(beatmap_ref01_data_raw[0][1])

        # LOAD
        beatmap_ref01_ent = client.Beatmap(None)
...
def _beatmap_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/beatmap/BeatmapTestData.json")
