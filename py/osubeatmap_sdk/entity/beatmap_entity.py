...
from osubeatmap_sdk.osubeatmap_types import (
    Beatmap,
    BeatmapLoadMatch,
)
...
class BeatmapEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BeatmapEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Beatmap:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Beatmap().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
