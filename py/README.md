...
The Python SDK for the OsuBeatmap API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Beatmap(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.OsuBeatmap(None).load(
    {"id": "test01"}, None
)
