...
class BeatmapEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BeatmapEntity($this->_client, $opts);
    }

    /**
     * @param Beatmap|array $args Beatmap data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OsuBeatmapHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Beatmap|array The current Beatmap data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Beatmap fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OsuBeatmapHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Beatmap fields).
     */
     ...
     *
     * @param BeatmapLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed BeatmapLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Beatmap|array The loaded Beatmap as an assoc-array at the
     *   SDK boundary; throws OsuBeatmapError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = OsuBeatmapHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
