...
class FtCtrl extends OsuBeatmapControl
{
...
class FtClient
{
    ...
    private OsuBeatmapUtility $utility;

    public function __construct(OsuBeatmapUtility $utility, array $options)
    {
    ...
    public function get_utility(): OsuBeatmapUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OsuBeatmapContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OsuBeatmapContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OsuBeatmapContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OsuBeatmapConfig::make_feature($name);
    ...
    private static function buildUrl(OsuBeatmapSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OsuBeatmapError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OsuBeatmapResponse($fetched);
        }
    ...
    private function fail(OsuBeatmapContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OsuBeatmapContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
