...
[MCP](https://modelcontextprotocol.io) server exposing the OsuBeatmap SDK as
two agent tools — `osu-beatmap_list` and `osu-beatmap_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/osu-beatmap-mcp)
make build

# 2. Provide credentials via the environment
export OSU_BEATMAP_APIKEY=sk_live_xxx
...
```jsonc
// osu-beatmap_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// osu-beatmap_load: one record by id
{ "entity": "beatmap", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/osu-beatmap-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user osu-beatmap \
     -- "$PWD"/dist/*/osu-beatmap-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `osu-beatmap_list` and `osu-beatmap_load` tools now appear
   in new sessions. Ask the agent to *"list search using osu-beatmap"*
   and it calls `osu-beatmap_list` with `{"entity":"search"}`.
...
```sh
export OSU_BEATMAP_APIKEY=sk_live_xxx            # API key
export OSU_BEATMAP_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `osu-beatmap_list` tool
...
### Call the `osu-beatmap_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `osu-beatmap_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `osu-beatmap_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OSU_BEATMAP_APIKEY` | API key sent with every request. |
| `OSU_BEATMAP_BASE` | Optional override of the API base URL. |
