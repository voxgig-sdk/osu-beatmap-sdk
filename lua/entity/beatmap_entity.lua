...
local BeatmapEntity = {}
BeatmapEntity.__index = BeatmapEntity


function BeatmapEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BeatmapEntity)
  self._name = "beatmap"
  self._client = client
...
function BeatmapEntity:get_name()
  return self._name
end


function BeatmapEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BeatmapEntity.new(self._client, opts)
end
...
function BeatmapEntity:mark_deleted()
  self._deleted = true
end


function BeatmapEntity:deleted()
  return true == self._deleted
end


function BeatmapEntity:data_set(args)
  if args ~= nil then
...
function BeatmapEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BeatmapEntity:match_set(args)
  if args ~= nil then
...
function BeatmapEntity:match_get()
...
function BeatmapEntity:stream(action, args, callopts)
...
---@param reqmatch BeatmapLoadMatch
---@param ctrl? table
---@return Beatmap
---@return string? err
function BeatmapEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function BeatmapEntity:_run_op(ctx, post_done)
  local utility = self._utility
