...
The Lua SDK for the OsuBeatmap API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Beatmap(nil):load({ id = "example_id" }, nil)
...
local result, err = client:OsuBeatmap(nil):load(
  { id = "test01" }, nil
)
