...
//    only - a typed OsuBeatmapContext there would make every ctx
...
final class OsuBeatmapOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new OsuBeatmapContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new OsuBeatmapSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new OsuBeatmapResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new OsuBeatmapError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new OsuBeatmapResponse($clean['response']);
    ...
    public static function err_from_map($m): ?OsuBeatmapError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OsuBeatmapError(is_string($code) ? $code : '', $msg);
    }
