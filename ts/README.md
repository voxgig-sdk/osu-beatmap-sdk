...
```ts
try {
  const beatmap = await client.Beatmap().load({ id: 'example_id' })
  console.log(beatmap)
} catch (err) {
...
const beatmap = await client.Beatmap().load({ id: 'test01' })
...
await beatmap.load({ id: "example_id" })

// beatmap.data() now returns the loaded beatmap data
// beatmap.match() returns { id: "example_id" }
```
