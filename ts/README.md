# OsuBeatmap TypeScript SDK



The TypeScript SDK for the OsuBeatmap API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Beatmap()` — each with a small set of operations (`list`, `load`)
...
| `tester(testopts?, sdkopts?)` | `OsuBeatmapSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `OsuBeatmapSDK.test(testopts?, sdkopts?)` | `OsuBeatmapSDK` | Create a test-mode client. |
...
| `client` | `client(): OsuBeatmapSDK` | Return the parent SDK client. |
...
| `beatmapset_id` | `number` | Beatmap set ID |
...
| `max_combo` | `number` | Maximum combo |
| `mode` | `number` | Game mode (0=osu!, 1=Taiko, 2=Catch, 3=Mania) |
...
```ts
const download = await client.Download().load({ id: 1 })
```
...
osu-beatmap/
├── src/
│   ├── OsuBeatmapSDK.ts        # Main SDK class
