...
//     (\`new OsuBeatmapSDK(...)\` / \`OsuBeatmapSDK.test(...)\` become
//     \`OsuBeatmapSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"beatmap":{"test01":{"id":"test01"}},"download":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OsuBeatmapSDK(...)\` and any
// \`OsuBeatmapSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OsuBeatmapSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OsuBeatmapSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
