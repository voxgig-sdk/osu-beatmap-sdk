...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.OSUBEATMAP_TEST_LIVE ||
    'TRUE' === process.env.OSUBEATMAP_TEST_OVERRIDE
  ) {
  ...
  m.OSUBEATMAP_TEST_EXPLAIN = process.env.OSUBEATMAP_TEST_EXPLAIN || m.OSUBEATMAP_TEST_EXPLAIN
